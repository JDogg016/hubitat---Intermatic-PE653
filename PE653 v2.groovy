    ...
    */
    ...
    private List setSpaSetpointInternal(Integer degrees) {
    ...
    /*
    def zwaveEvent(hubitat.zwave.commands.multiinstancev1.MultiChannelCmdEncap cmd) {
    ...
    */
    ...
    private encap(cmd, endpoint) {
    	...
    	if (endpoint) {
    //		zwave.multiinstancev1.multiChannelCmdEncap(destinationEndPoint:endpoint, sourceEndPoint: endpoint).encapsulate(cmd)
    		zwave.multiinstancev1.multiInstanceCmdEncap(instance: endpoint).encapsulate(cmd)
    	} else {
    		cmd
    	}
    }
    ...
    private List refreshLight() {
    	...
    	for (int i=1;i<=5;i++) {
    //		cmds << zwave.multiinstancev1.multiChannelCmdEncap(destinationEndPoint:i, sourceEndPoint: i).encapsulate(zwave.switchBinaryV1.switchBinaryGet())
    ...
    */
    ...
    //log.trace "M1Sw1=${M1Sw1} M1Sw2=${M1Sw2} M1Sw3=${M1Sw3} M1Sw4=${M1Sw4} M1Sw5=${M1Sw5} M1Mode=${M1Mode} M1Temp=${M1Temp} M1VSP=${M1VSP}"
        ...
        if (MxMode == "1") {
            ...
            cmds.addAll(setPoolModeInternal())
            cmds.addAll(setPoolSetpointInternal(MxTemp.toInteger()))
        } else if (MxMode == "3") {
            cmds.addAll(setSpaModeInternal())
        } else if (MxMode == "4") {
            cmds.addAll(setSpaModeInternal())
            cmds.addAll(setSpaSetpointInternal(MxTemp.toInteger()))
        }
    ...
    def on() {
    	log.debug "+++++ on()"
        delayBetweenLog([
    //		zwave.multiinstancev1.multiChannelCmdEncap(sourceEndPoint: 1, destinationEndPoint: 1, commandClass:37, command:1, parameter:[255]),
    //		zwave.multiinstancev1.multiChannelCmdEncap(sourceEndPoint: 1, destinationEndPoint: 1, commandClass:37, command:2)
    ...
    def off() {
    	log.debug "+++++ off()"
        delayBetweenLog([
    //		zwave.multiinstancev1.multiChannelCmdEncap(sourceEndPoint: 1, destinationEndPoint: 1, commandClass:37, command:1, parameter:[0]),
    //		zwave.multiinstancev1.multiChannelCmdEncap(sourceEndPoint: 1, destinationEndPoint: 1, commandClass:37, command:2)
    ...
    def delayBetweenLog(parm, dly=DELAY) {
        ...
        lst.each {l ->
    log.trace "l -> ${l}"
    		if (l instanceof hubitat.device.HubAction) {log.trace "instanceof hubitat.device.HubAction"}
    //        if (l instanceof hubitat.zwave.commands.associationv2.AssociationGroupingsGet) {log.trace "$l instanceof hubitat.zwave.commands.associationv2.AssociationGroupingsGet"}
    		//if (l instanceof hubitat.HubAction) {log.trace "delay instanceof hubitat.zwave.commands.associationv2.AssociationGroupingsGet"}
